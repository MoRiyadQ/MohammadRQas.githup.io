        ...
        for line in output_lines:
            if line.strip() and "Dirs" not in line:  # Skip header lines
                ...
                if len(parts) > 1:
                    ...
                    gobuster_results.append({"path": path, "status": status})
...
def nikto():
    ...
    if request.method == "POST":
        ...
        # Strict whitelist validation - only allow nikto with specific parameters
        valid_nikto_pattern = r"^nikto\s+(-[h|u|T|p|v|x|F]\s+[a-zA-Z0-9\.\-\_\/\:]+\s*)*$"
        disallowed_chars = r"[\|\&\;\`]"  # Disallow shell special characters
...
def download_report(scan_id):
    ...
    add_toc_entry("8. Testing Methodology", 1, "testing_methodology")
    add_toc_entry("   • Reconnaissance", 2, "reconnaissance")
    add_toc_entry("   • Methodology", 2, "methodology_details")

    add_toc_entry("9. Conclusion", 1, "conclusion")
    doc.add_page_break()
   ...
   # Findings Summary Section
    ...
    # Add Testing Methodology section after Cyber Security Design Principles
